...
async def generate_daily_report(
    ...
    Returns {"markdown": str, "model": str} — the report (with a model stamp
    footer) and the resolved model name that generated it.
    """
    logger.info(
...
Do not add new sections. Return only the completed markdown — no preamble."""
    ...
    markdown = (markdown or "") + (
        f"\n\n---\n*Generated by `{resolved_model}` via {provider} on {stamp} ({triggered_by}).*"
    )
    ...
    try:
        ...
        logger.info("Daily report persisted user_id=%s triggered_by=%s model=%s",
                    user_id, triggered_by, resolved_model)
    except Exception as e:
