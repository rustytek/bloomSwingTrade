...
# Runs once before the service starts. Reads add-on options from HA's
# /data/options.json via bashio and writes a secured env file for the service.
# ==============================================================================
set -e
...
# /data is HA's persistent volume for this add-on (survives updates/restarts).
mkdir -p /data/ssl
chmod 700 /data/ssl
...
# ── SSL setup -----------------------------------------------------------------
# Option A: use HA's own certificate (Let's Encrypt via the HA SSL add-on)
# Option B: fall back to a self-signed cert generated by generate_ssl.py
if bashio::config.true 'use_ha_ssl'; then
    ...
    bashio::log.info "Using self-signed SSL certificate (generated on first run)"
    SSL_CERT="/data/ssl/cert.pem"
    SSL_KEY="/data/ssl/key.pem"
fi

# ── Write env file (chmod 600 — readable only by root inside container) -------
...
bashio::log.info "SwingTrader configuration complete. Starting service..."
