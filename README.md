...
| **Strategy Lab** (`/backtest`) | Does this strategy actually work? A strategy × regime **edge matrix** with Wilson confidence intervals and `confirmed` / `unproven` / `mis-tagged` verdicts (each needing a multiple-testing-corrected significance test and agreement between pre-2019 and 2019+ data), a **Deflated Sharpe** on every run that allows for how many variants you've tried, plus walk-forward backtests in two modes over any **5-year window of the last 20 years**, filled the way you actually trade (next-day limit entries, never at the signal close) (so a test can sit on 2008, 2018, 2020 or 2022). The edge matrix uses all 20 years. |
...
LiteLLM is the recommended path — this app talks to it exclusively for AI analysis and never calls a model runtime directly. Use a stable tier alias for `AI_MODEL`/`REPORT_MODEL`, not a raw model name, so the physical model behind it can change without editing this config.
