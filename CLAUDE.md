...
- `/backtest` — the **Strategy Lab** (`static/backtest.html`). Headlined by the strategy × regime edge matrix, a **20-year price history** panel (coverage from `GET /api/history/status`, admin-only download button), then a single-strategy walk-forward with a rotation/trade_plan mode toggle, sortable trade log with exit-kind distribution, and a severity-sorted caveats panel.
...
| `services/long_history.py` | The 20-year archive: `splice()` joins archive bars (older) onto the StockCache (recent) and **rebases** the archive onto the cache's adjustment basis using shared dates (a split/dividend after the backfill would otherwise be a fake jump; inconsistent ratios → archive not used, reported). `run_backfill()` (job worker only) downloads SPY and ^VIX first, then the universe + watchlists; resumable, never shrinks a stored series, retries problem tickers at most daily, refreshes ^VIX whenever it falls behind (it is not in the daily cache). `status()` feeds the Strategy Lab panel |
| `services/history_sources.py` | Fetchers for the backfill: `fetch_yfinance` (primary) and `fetch_tiingo` (fallback; ADJUSTED fields; `brk-b` symbols; no indices; 429 → `TiingoRateLimited`, and the run stops using Tiingo — free tier is 50 requests/hour). `assess()` rejects empty / truncated (starts later than the ticker's known 5-year cache start) / stale / >2%-holed downloads; `better()` picks between sources |
| `api/history.py` | `GET /api/history/status` (coverage, sources, problems, last run), `POST /api/history/backfill` (**admin only** — the archive is shared; one download at a time across all users) |
...
**A refetch can never shrink the cache.** `get_history` is the only writer of `StockCache.history_json`; before writing it runs `history_fetch_rejection(cached, fetched)` and keeps the cached bars (without marking them fresh, so the next cycle retries) when Yahoo returns nothing, fewer than `MIN_REFETCH_RATIO` (90%) of the cached bars, or a series that ends earlier than the cache. A single empty Yahoo answer used to overwrite five years of bars — BK, HOLX, MMC and SEE were found with 0 bars, EA/EQR with 1, and silently dropped out of every backtest.
...
| `services/job_worker.py` | `python -m services.job_worker <job_id>` — a **fresh interpreter with its own GIL**. Must have no import side effects (it runs per job) and must never import `main.py`. Every path ends in a terminal status. Kinds: `edge_matrix`, `daily_report`, `history_backfill` (the 20-year download), and a `selftest` kind that exercises the whole pipeline in seconds |
...
- `rotation` — the original equal-weight top-N rotation with a turnover cost, marked to market only on rebalance dates. It is a **frozen regression surface**: `test_backtest.py` pins its output to a recorded digest that was diffed against the pre-change engine. Do not change it without re-recording that digest deliberately. (Re-recorded once, 2026-09-25, for the next-session fill convention — the per-period picks were verified identical on real data; only fill prices changed. The history is in the comment above `_ROTATION_BASELINE_DIGEST`.)
...
**Trades fill in the session AFTER the decision (ML4T 3e §16.2 — same-bar execution "fuses past and future").** Decisions read a bar's close; nothing trades at that close. trade_plan entries are day limit orders at `plan["entry_zone"][1]` (the top of the ATR zone — the same limit the Weekly Plan tells you to place) for the next session: `next_day_limit_fill()` fills at `min(open, limit)`, and returns no fill on a gap above the limit (`entries_missed_gap`) or a fill at/below the planned stop (`entries_missed_through_stop`); orders from the last decision date never fill (`entries_missed_no_session`). If the fill session trades through the stop, the position is stopped out that day with no same-day target credit. Time-stop and regime exits (`DEFER_TO_NEXT_OPEN`) are decided at a close and sell at the next open (`OpenPosition.pending_exit`); stops, targets and partials remain resting intrabar orders. Rotation buys and sells at the open of the session after each decision (`_next_open`), SPY benchmark likewise; the final period is marked at its close. The SPY regime gate is a decision and still reads the close. Every run carries a `fill_convention` caveat (plus `entries_missed` when orders went unfilled). Measured on the real DB when introduced: momentum trade_plan CAGR 13.6→9.5 %, breakout rotation 2.2→0.1 %, pullback slightly up — i.e. the old same-close fills flattered the breakout/momentum family most.

**No look-ahead is the cardinal rule.** Entries evaluate `strategy.candidate(bars, idx)` (which reads only `bars[:idx+1]`) and fill in the next session (above); exit rules are handed one enriched bar at a time and never the series. `test_backtest.py` asserts that truncating the series immediately after a decision bar does not change the decision made at that bar. Never relax this.
...
**Data depth vs test window — the database keeps 20 years, a Strategy Lab run tests at most 5.** `run_walk_forward_backtest(..., history="5y"|"20y", progress=None, max_window_years=MAX_WINDOW_YEARS)`. `history` is which DATA the run may draw on: `5y` = the StockCache only; `20y` = `services/long_history.py` per ticker (archive + cache, rebased), falling back to the cache for any ticker without an archive. The TEST WINDOW is capped at `MAX_WINDOW_YEARS` (5): with a `start_date` the window is start → start + min(period, 5y) (an explicit `end_date` cannot exceed it); without one it is the most recent 5 years. `GET /api/backtest/walk-forward` always passes `history="20y"`, so a user picks *which* 5 years by start date (e.g. 2007-01-01 to test through 2008) and the run still answers in the request: `_BarLoader.trim_before` loads each ticker only from `warmup + _PRE_WINDOW_EXTRA_BARS` (400) bars before the window — pinned by a test to give results identical to loading everything. Period presets are `1Y`/`2Y`/`5Y`/`all` (= 5 years). **The edge matrix passes `max_window_years=None`** — it measures every regime across all 20 years and runs in the job worker. The window cap is the user's decision (2026-09-24): do not lift it for interactive runs without asking; an uncapped 20-year run is minutes of CPU and would have to move to the worker.

**The engine scans one ticker at a time.** 20 years × ~540 tickers as Python dicts is ~1.25 GB (460 B/bar measured) — too much for the HA host (8 GB shared with Whisper, Omada, HA). So the run first loads each ticker alone, computes `strategy.candidate()` for every rebalance date (it only reads that ticker's `bars[:idx+1]`, so this is exactly equivalent), keeps only the top `k_keep` per date via `_keep_top()` (stable sort — identical tie order), then simulates; only tickers actually bought are reloaded, through a 24-entry LRU (`_BarLoader`). Verified identical to the previous all-in-memory engine for every actionable strategy in both modes on real data, and the frozen rotation digest still matches. Date lookups (`_value_on_or_before` etc.) are `bisect`-based (the old linear scan was ~25% of a 5-year run).

**Real VIX in regime tagging.** When the archive has ^VIX for a date, the crisis flag is `VIX >= regime.VIX_CRISIS` (25, the live rule); otherwise the 20-day realized-volatility proxy (≥ 22) as before. No archive → identical to before.
...
| `TIINGO_API_KEY` | *(empty)* | Optional free Tiingo key (`tiingo_api_key` add-on option) — fallback source for the 20-year backfill when Yahoo's download is empty/truncated/holed |
